...
class Service extends React.Component {
  ...
  componentDidMount() {
    ...
    order.getServiceOrderList().then(res => {
      this.setState({
        list: res.list,
      })
    })
    ...
    return list.map(item => {
      return (
        <React.Fragment key={item.id}>
          <Card
            onClick={() => history.push(`/order/service/detail/${item.id}`)}
          >
            <Card.Header
              thumb={item.goods_img}
              title={item.name}
              extra={this.statusLabel(item.service_status, item.status)}
            />
            <Card.Body>
              ...
              <Flex>
                <Flex.Item>
                  <div className="font-label">
                    总价：<span className="font-value">{item.price}</span>
                  </div>
                </Flex.Item>
                <Flex.Item>
                  <div className="font-label">
                    购买数量：
                    <span className="font-value">{item.goods_num}</span>
                  </div>
                </Flex.Item>
              </Flex>
            </Card.Body>
