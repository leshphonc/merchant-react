...
import { Flex, Card, Picker, Button, Toast, InputItem } from 'antd-mobile'
...
class GroupOrderDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    order.fetchGroupOrderDetai(match.params.orderId).then(() => {
      ...
      const expressList = []
      groupOrderDetail.express_list.forEach((item, index) => {
        if (item.ids === groupOrderDetail.distribution.express_type) {
          this.setState({
            expressIndex: index,
          })
        }
      ...
      this.setState({
        ...
        expressId:
          groupOrderDetail.distribution.express_type || expressList[0].value,
        expressName:
          expressList.length > 0 ? expressList[expressIndex].label.label : '',
      ...
      <React.Fragment>
        ...
        {detail.store_id === '0' && storeList.length > 0 && (
          <Card style={{ marginTop: '10px' }}>
            ...
            />
            <Card.Body
              style={{ color: '#666', fontSize: 15, textAlign: 'center' }}
            >
              <Picker
                data={storeList}
                cols={1}
        ...
        {detail.store_id && distribution.adress && (
          <div>
            <Card style={{ marginTop: '10px' }}>
              ...
              <Card.Body style={{ color: '#666', fontSize: 15 }}>
                <Flex>
                  <div style={styleSpan.spanLeft}>
                    收件人: {distribution.contact_name}
                  </div>
                  <div style={styleSpan.spanRight}>{distribution.phone}</div>
                </Flex>
                <Flex>
                  <div style={{ marginTop: '10px' }}>
                    收货地址：{distribution.adress}
                  </div>
                </Flex>
              </Card.Body>
            </Card>
                ...
                <Flex style={styleSpan.topDis}>
                  <div style={{ width: '100%' }}>
                    <Button
                      ...
                      onClick={() => {
                        order
                          .verificGroupAll(detail.order_id, detail.store_id)
                          .then(res => {
                            if (res)
        ...
        <Card style={{ marginTop: '10px' }}>
          ...
          <Card.Body style={{ color: '#666', fontSize: 15 }}>
            <Flex>
              <div>
                {detail.delivery_comment ? detail.delivery_comment : '暂无'}
              </div>
            </Flex>
          </Card.Body>
        </Card>
