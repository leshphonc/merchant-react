...
class ServiceSingleRecord extends React.Component {
  ...
  componentDidMount() {
    ...
    commodity.fetchSingleRecord(match.params.id, page).then(res => {
      this.setState({
        list: res.list,
      })
    })
  ...
  mapList = () => {
    ...
    return list.map(item => (
      <React.Fragment key={item.id}>
        <Card>
          <Card.Header thumb={item.goods_img} title={item.name} />
          <Card.Body>
  ...
  loadMore = () => {
    ...
    commodity.fetchSingleRecord(match.params.id, page + 1).then(res => {
